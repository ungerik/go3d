...
func TestNormalizeEdgeCases(t *testing.T) {
	tests := []struct {
		...
		{"tiny vector (below epsilon)", T{1e-16, 1e-16}, false},
		{"already normalized", T{1, 0}, true},
		{"nearly normalized positive deviation", T{1.00000000001, 0}, true},
		{"nearly normalized negative deviation", T{0.99999999999, 0}, true},
		{"nearly normalized mixed", T{0.7071067811865476, 0.7071067811865475}, true}, // ~sqrt(2)/2 components
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.checkLength {
				...
				if !PracticallyEquals(length, 1.0, 0.00001) {
...
func TestNormalizedEdgeCases(t *testing.T) {
	tests := []struct {
		...
		{"zero vector", T{0, 0}, false},
		{"tiny vector", T{1e-16, 1e-16}, false},
		{"already normalized", T{1, 0}, true},
		{"needs normalization", T{3, 4}, true},
	}
