...
func TestLeftRightWinding(t *testing.T) {
	...
	for angle := 0; angle <= 360; angle += 15 {
		rad := (math64.Pi / 180.0) * float64(angle)

		bx := float32(clampDecimals(math64.Cos(rad), 4))
		by := float32(clampDecimals(math64.Sin(rad), 4))
		b := T{bx, by}
...
func clampDecimals(decimalValue float64, amountDecimals float64) float64 {
	factor := math64.Pow(10, amountDecimals)
	return math64.Round(decimalValue*factor) / factor
}
